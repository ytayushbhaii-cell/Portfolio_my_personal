...
export function ProjectCard({ project }: { project: Project }) {
  ...
  if (type === "android" && apkUrl) cta = { href: apkUrl, label: "Download APK", icon: <Download className="h-3.5 w-3.5" /> };
  else if (type === "website" && project.website_url) cta = { href: project.website_url, label: "Visit Site", icon: <ExternalLink className="h-3.5 w-3.5" /> };
  else if (type === "telegram" && project.bot_url) cta = { href: project.bot_url, label: "Open Bot", icon: <Bot className="h-3.5 w-3.5" /> };
      ...
      <Link to="/projects/$slug" params={{ slug: project.slug }} className="block">
        <div className="relative aspect-[16/10] overflow-hidden bg-secondary">
          {project.thumbnail_url ? (
            ...
            <div className="grid h-full w-full place-items-center text-3xl font-bold gradient-text">
        ...
        {project.tech_stack.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {project.tech_stack.slice(0, 4).map((t) => (
              <span key={t} className="rounded-full bg-secondary px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                {t}
              </span>
            ))}
          </div>
